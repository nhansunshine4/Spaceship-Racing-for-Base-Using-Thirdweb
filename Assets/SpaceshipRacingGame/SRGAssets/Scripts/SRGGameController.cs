...
//CALCULATING section CHANCES
			...
			// Calculate the ratio between the object's scale and its tiling. This is used to set the offset speed of the texture
			if ( tubeObject )    offsetRatio = new Vector2( -tubeObject.thisRenderer.material.mainTextureScale.x/tubeObject.thisTransform.localScale.x, -tubeObject.thisRenderer.material.mainTextureScale.y/tubeObject.thisTransform.localScale.z);
		...
		void  Update()
		{
			...
			else
			{
				...
				if ( isPaused == false )
				{
					...
					tubeObject.thisTransform.position = new Vector3( tubeObject.thisTransform.position.x, tubeObject.thisTransform.position.y, playerController.position.z);

					// Animate the material of the tube to give an illusion of movement through it
					tubeObject.thisRenderer.material.mainTextureOffset = new Vector2( tubeObject.thisRenderer.material.mainTextureOffset.x, offsetRatio.y * tubeObject.thisTransform.position.z * 0.006f);
